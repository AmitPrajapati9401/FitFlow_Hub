...
import { 
  FitFlowLogo, LoadingIcon, CameraOffIcon, EyeIcon, CheckIcon, CloseIcon, CameraIcon, SparklesIcon
...
const Login: React.FC<LoginProps> = ({ onLogin, onNavigateToSignUp }) => {
  ...
  const handleDemoLogin = async () => {
    ...
    setError(null);
    try {
      const user = await db.login('demo@fitflow.com');
      if (user) {
        onLogin(user);
      }
    } catch (e) {
      ...
      <div className="min-h-screen text-white bg-zinc-950 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-sm mx-auto text-center">
          ...
          <button
            ...
            className="mt-6 w-full py-4 border border-white/10 rounded-full flex items-center justify-center gap-2 text-zinc-400 hover:bg-white/5 hover:text-white transition-all font-black text-xs uppercase tracking-widest"
          >
