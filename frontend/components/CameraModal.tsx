...
const CameraModal: React.FC<CameraModalProps> = ({ onClose, onCapture }) => {
    ...
    const handleCapture = () => {
        if (videoRef.current && canvasRef.current && isCameraReady) {
            ...
            if (context) {
                // Flip the image horizontally since the video feed is mirrored
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 relative max-w-2xl w-full">
                ...
                <div className="relative aspect-video bg-zinc-800 rounded-lg overflow-hidden">
                    {/* The video element is mirrored using scaleX(-1) to provide a more natural selfie experience */}
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80">
                            {error ? (
                                <div className="text-center p-4">
                                    ...
                                    <button onClick={startCamera} className="mt-4 flex items-center mx-auto gap-2 bg-lime-300 text-black font-bold py-2 px-4 rounded-full"><RetryIcon /> Retry</button>
                                </div>
                            ) : (
                                <LoadingIcon />
                            )}
                ...
                <div className="mt-4 flex justify-center">
                    <button onClick={handleCapture} disabled={!isCameraReady || !!error} className="flex items-center gap-2 bg-lime-300 text-black font-bold py-3 px-6 rounded-full disabled:bg-zinc-600 disabled:cursor-not-allowed"><CaptureIcon /> Capture</button>
                </div>
            </div>
        </div>
    );
