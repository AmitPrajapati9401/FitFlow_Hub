...
const PoseOverlay: React.FC<PoseOverlayProps> = ({ landmarks, videoRef, activeLandmarks, isCorrectForm = true }) => {
    ...
    if (!landmarks || videoDimensions.width === 0) return null;
    ...
    const glowClass = isCorrectForm ? "drop-shadow-[0_0_8px_rgba(163,230,53,0.8)]" : "drop-shadow-[0_0_8px_rgba(251,191,36,0.6)]";

    return (
        <svg
            className={`absolute top-0 left-0 w-full h-full pointer-events-none z-10 transform scaleX(-1) transition-all duration-300 ${glowClass}`}
