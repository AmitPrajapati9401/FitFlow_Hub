...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  // Safe access to current slide with fallback to prevent undefined errors
  const current = slides[currentSlide] || slides[slides.length - 1];

  const handleNext = async () => {
    if (!current) return;
    
    if (current.id === 'camera' && !isCameraReady) {
